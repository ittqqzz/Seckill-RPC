...
* 后端
  ...
  * 后端服务器应该独立，不要和其他业务共用，本项目不是分布式的，但是可以使用分布式架构进一步缓解后端压力

  * 进一步降低后端压力，后端模块因该是分为两部分，一部分处理前端请求并投递消息，另一部分就慢慢消费消息并通知用户处理结果

* 上游优化的好的话，mysql 单机可以应付过来

## Q&A

* 怎么控制用户访问秒杀连接？
    * 第一次访问，请求的是接口暴露服务，如果秒杀开始了就返回执行秒杀的 URL，如果秒杀未开始或者结束了，返回秒杀未开始/已结束
    * 第二次访问，此时在访问的要么是执行秒杀的 URL，要么是请求暴露接口，请求暴露接口此时无需再访问后端了，因为第一次访问的时候会把秒杀的时间返回给前端缓存；要么是访问的是执行秒杀，还是由前端判断，是否在第一次得到了暴露的接口然后继续处理。 
...
* 哪里会出现并发？怎么优化并发？
    1. 可能会出现并发的地方，红色的部分[![FHhayF.png](https://s2.ax1x.com/2019/01/06/FHhayF.png)](https://imgchr.com/i/FHhayF)
    2. 获取系统时间无需优化
    3. 获取页面需要优化，将页面资源放到 CDN 上，同时控制按钮不允许多次点击
    4. 秒杀地址接口需要优化，应该放到redis里面
    ...
    5. 优化要分为前后端
        ...
        2. **后端（不要让请求落到数据库上去）**
            1. 一般都要先登录再秒杀，所以每个用户都有ID，对同一个用户ID，每5秒允许通过一个请求
            2. 使用请求队列（MQ），设置队列长度，只卖1万台手机，就只允许队列里面存在1万个请求。多余的抛弃，返回重试/售罄。
                或者1W台手机每次只允许1千个请求进入，处理完一批再放下一批请求。直到售罄。
            3. 热点数据Redis缓存起来，例如：一旦秒杀商品放到网页上了，那么它的秒杀开始时间、结束时间、价格等信息都是不会变更了的，用户浏览这些信息的时候用Redis缓存起来最好。
            4. （分时间段秒杀，类似于12306分时段放票。可借鉴。。）
            5. 业务逻辑的异步：例如 下单业务与支付业务的分离
            6. 成功进入MQ的请求先创建订单并计时，然后扣减库存，若超时未支付就丢弃订单并恢复库存。                                    
            7. 对与数据库：
                1. 只要上游过滤了，数据库单机处理没问题
                2. 不然再分库分表
            8. 架构图（上面已经展示了）
...
* 系统并发时为何低效：
  其实 mysql 不低效，对同一行数据做 update，每秒可以执行 4W 次。
  java 的执行也不低效。
